...
const InfluencerComponent = () => {
    ...
    const [influenceValue, setInfluenceValue] = useState(50);
    ...
    const applyInfluence = async () => {
        if (!selectedSong) {
            alert("Please select a song to influence!");
            return;
        }
    
        console.log("Applying influence for song:", selectedSong);
        
        try {
            let apiUrl = `http://localhost:5001/api/apply-influence?song=${selectedSong}&influence=${influenceValue}`;
        
            const response = await fetch(apiUrl);
            const data = await response.json();
            // Update any relevant state based on the response if needed
        ...
        <Container className="mt-4 influencer-component">
            ...
            <Row className="mb-4 justify-content-center">
                <Col md={4} className="text-center">
                    ...
                    <BootstrapButton onClick={applyInfluence} size="lg" className="btn-apply-influence">Apply Influence</BootstrapButton>
                </Col>
            </Row>
