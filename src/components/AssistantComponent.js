...
const searchDatabase = async (inferredMood) => {
  try {
      const moodsArray = inferredMood.map(mood => mood.split('.').slice(1).join('.').trim());
      const response = await axios.post('http://localhost:5001/api/search-mood', { moodsFromGPT: moodsArray });
...
const AssistantComponent = () => {
    ...
    const handleSearch = async () => {
      try {
          ...
          console.log("Sending inferred mood to backend:", inferredMood);
          const matchingSongs = await searchDatabase(inferredMood); // No need to split inferredMood
