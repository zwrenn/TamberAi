...
const SpotifyPlayer = ({ trackUri }) => {
    ...
    useEffect(() => {
        if (trackUri && isPlayerReady) {
            ...
            console.log("Track URI or player readiness check failed. Skipping playback attempt.");
        }
    ...
    const handleLike = () => {
        ...
        if (trackUri) {
            ...
            .then(response => {
                if (response.ok) {
                    console.log(isLiked ? 'Track removed from Liked Songs' : 'Track added to Liked Songs');
                } else {
                    setIsLiked(!isLiked);
    ...
    return (
        <div className="spotify-player">
            <div className="track-info">
                ...
                />
                <img src={trackDetails.albumCover} alt="Album Cover" className="album-cover"/>
            ...
            <div className="auth-controls">
                <a href="https://accounts.spotify.com/authorize?client_id=e74b4171efe74283b6e4f39d226eb8a7&redirect_uri=http://localhost:3000/&scope=streaming user-read-email user-read-private user-library-modify&response_type=token" className="spotify-login">Login with Spotify</a>
            </div>
        </div>
    ); 
}
